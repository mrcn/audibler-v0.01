...
<package xmlns="http://www.idpf.org/2007/opf" version="2.0" unique-identifier="BookID">
  <metadata xmlns:dc="http://purl.org/dc/elements/1.1/" xmlns:opf="http://www.idpf.org/2007/opf">
    <dc:title>Beating Seasonal Depression: A Budget Light Therapy Guide</dc:title>
    <dc:creator>SAD Light Therapy Resource</dc:creator>
    <dc:language>en</dc:language>
    <dc:date>{datetime.now().strftime('%Y-%m-%d')}</dc:date>
    <dc:identifier id="BookID">urn:uuid:sad-light-therapy-guide-2026</dc:identifier>
    <dc:description>A comprehensive, budget-focused guide to treating Seasonal Affective Disorder using light therapy, specifically designed for Cleveland winters and dark climates.</dc:description>
  </metadata>
  <manifest>
...
<ncx xmlns="http://www.daisy.org/z3986/2005/ncx/" version="2005-1">
  <head>
    <meta name="dtb:uid" content="urn:uuid:sad-light-therapy-guide-2026"/>
    <meta name="dtb:depth" content="{max_depth}"/>
    <meta name="dtb:totalPageCount" content="0"/>
    <meta name="dtb:maxPageNumber" content="0"/>
  </head>
  <docTitle>
    <text>Beating Seasonal Depression: A Budget Light Therapy Guide</text>
  </docTitle>
  <navMap>
...
<html xmlns="http://www.w3.org/1999/xhtml">
<head>
  <title>Beating Seasonal Depression: A Budget Light Therapy Guide</title>
  <style type="text/css">
    body {{
      font-family: Georgia, serif;
      line-height: 1.6;
...
def markdown_to_html(md_text, toc_entries):
    """Simple markdown to HTML conversion with anchored headers and links."""
    html = md_text
    ...
    # Convert markdown links [text](url) to HTML - do this BEFORE bold/italic
    # This preserves the URL properly
    def replace_link(match):
        text = match.group(1)
        url = match.group(2)
        # Unescape the URL since we already escaped everything
        url = url.replace('&amp;', '&')
        return f'<a href="{url}">{text}</a>'

    html = re.sub(r'\[([^\]]+)\]\(([^)]+)\)', replace_link, html)

    # Bold and italic (process after links)
    ...
    for line in lines:
        ...
        if table_match:
            # Close paragraph or list if open
            if in_paragraph:
        ...
        # Close table if we were in one and this isn't a table line
        if in_table and not table_match:
            processed_lines.append('</table>')
            in_table = False

        # Check if line is a list item (starts with - or * or checkbox)
...
if __name__ == '__main__':
    create_epub(
        'sad-light-therapy-guide.md',
        'sad-light-therapy-guide.epub'
    )
